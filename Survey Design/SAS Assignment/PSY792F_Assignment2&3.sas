...
/*CHECK, this should NOT print any observatins. Makign sure all cases less than 1959 are not included.*/
PROC PRINT DAta= FINAL_1959;
	where BIRTHYR >1959;
RUN;						
	

/*Mean & SD of Age*/
PROC MEANS  data = FINAL_1959 ;
	VAR NAGE;
RUN;


/*Demographic characteristics*/
PROC FREQ DATA = FINAL_1959;
	TABLES FEMALE RACE HISPANIC Degree2 NCOUPLE OCOUPLE;
RUN;


/*MEAN & SD of Depression in 2012 and 2014*/
proc means data = FINAL_1959;
	VAR CESD_12 CESD_14;
RUN;


/*t-tests for depression scores by gender in 12 and 14*/

proc ttest data = FINAL_1959;
	CLASS FEMALE;
      var CESD_12;
   run;
...
/*/*t-tests for depression scores by work status in 12 and 14*/
...
/*Correlation for Life Sat and Depression at 2012 and 2014*/

PROC CORR data = FINAL_1959;
	VAR CESD_12 self_health ;
RUN;


PROC CORR data = FINAL_1959;
	VAR CESD_14 self_health ;
RUN;



/*Correlation for Impulse scale and Depression at 2012 and 2014*/
...
PROC CORR data = FINAL_1959;
	VAR self_health Impulse;
RUN;

/*regression for Impulse scale and self-rated health at 2012 and 2014*/


PROC REG data = FInal_1959;
	MODEL self_health = Impulse;
RUN;

PROC REG data = FInal_1959;
	MODEL self_health = Impulse Nage race ocouple female DEGREE2;
RUN;
...
/*Regression for Impulsivity and self-rated health at 2014*/
...
/*Note: Green statements produced in log: These are because of the large sample size and the Proic reg/proc corr statments are not 
		producing all of the graphics. Ignore this, it is fine. -Neil*/
...
/*PROC FREQ data = Demographics;*/
/*	TABLES NAGE GENDER SCHLYRS DEGREE RACE HISPANIC NCOUPLE OCOUPLE;*/
/*	WEIGHT NWGTR; /*<- This is your weight variable... It helps to account for the toaltal in the population. You will get very different answers if you use this vs not using it.*/*/
/*RUN;*/


/*Below is a "Survey" proc. This is the general layout for running surveys that have cluster variables*/

/*PROC SURVEYFREQ data = DEMOGRAPHICS;*/
/*	CLUSTER NAMEOFCLUSTERVAR; /*<- This is naming the cluster*/*/
/*	STRATA NAMEOFSTRATAVAR;*/
/*	TABLES VAR1 VAR2 VAR1*VAR2;*/
/*	WEIGHT NAMEOFWEIGHTVAR;*/
/*RUN;*/

/*End descriptuon of PROC SURVEYFREQ*/
