...
```{r}
apa.cor.table(itemtotal, show.conf.interval = FALSE, filename = "cor.doc")
```
...
```{r}
model.2f <- 'f1 =~ NA*change1_1 + change1_2R + change1_3 + change1_4R + change1_5 + change1_6R + change1_7 + change1_8 + change1_9R
f1 ~~ 1*f1
            f2 =~ NA*change2_1 + change2_2R + change2_3 + change2_4 + change2_5R + change2_6 + change2_7R + change2_8 + change2_9R + change2_10
f2 ~~ 1*f2'
...
parallel = fa.parallel(change,
 ...
 fa = 'fa',
 n.iter = 50,
 main = "Scree plot of Belief in change scale: All Items",
 SMC = TRUE)
```
...
```{r}
model.2f6 <- 'f1 =~ NA*change1_1 + change1_4R + change1_5 + change1_6R + change1_8 + change1_9R
f1 ~~ 1*f1
            f2 =~ NA*change2_1 + change2_2R + change2_5R + change2_7R + change2_10
f2 ~~ 1*f2'
...
```{r}
set.seed(123)
parallel = fa.parallel(changefinal,
 fm = 'ml',
 fa = 'fa',
 n.iter = 50,
 main = "Scree plot of Belief in change scale: Reduced",
 SMC = TRUE)
...
```{r}
dataSS <- data2 %>%
  select(scalefull, scalegeneral, scalepersonal, scalePC, scaleLC)
```

## Correlation table for all variables 
```{r}
conv <- cor(dataSS, use = "pairwise.complete.obs")
conv
```

```{r}
apa.cor.table(conv, show.conf.interval = FALSE, filename = "convergent.doc")
```
